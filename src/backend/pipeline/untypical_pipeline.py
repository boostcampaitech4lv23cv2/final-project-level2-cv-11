...
class Untypical_Pipeline:
    ...
    def png2svg(self): 
        for folder_path in glob.glob(os.path.join(os.getenv("HOME"), 'tmp/*')):
            for png_path in glob.glob(os.path.join(folder_path, "generated_mxfont/*")):
                ...
                os.system(f'convert {png_path} -flatten pgm:| mkbitmap -f 64 -t 0.4 - -o -| \
    ...
    def svg2ttf(self):
        ...
        example_json_path = os.path.join(os.getenv("HOME"),"src/model/svg2ttf/example.json" )
        ...
        for folder_path in glob.glob(os.path.join(os.getenv("HOME"), 'tmp/*')):
            ...
            tmp = {}
            for svg_path in glob.glob(os.path.join(folder_path, "generated_mxfont_svg/*")):
