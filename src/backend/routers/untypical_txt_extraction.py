...
async def make_ocr_font(file: UploadFile = File(...)):
    ...
    # Font Classification
    font_cls_result, font_cls_result_all, font_color = Untypical_pipeline.untypical_font_classification(
        copy.deepcopy(ocr_result)
    )

    res = Results(ocr_result=ocr_result, font_result=font_cls_result, font_result_all = font_cls_result_all, font_color = font_color)
    return res
